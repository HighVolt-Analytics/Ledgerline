...
async def get_app_settings() -> ApiEnvelope[AppSettingsResponse]:
    ...
    return ApiEnvelope(
        data=AppSettingsResponse(
            ...
            public_api_base_url=s.public_api_base_url_resolved,
            stripe_mode=s.stripe_mode_normalized,
            xero_configured=s.xero_configured,
            quickbooks_configured=s.quickbooks_configured,
            stripe_global_payouts_enabled=s.stripe_global_payouts_enabled,
            stripe_global_payouts_access_status=s.stripe_global_payouts_access_status.strip().lower()
            or "not_requested",
