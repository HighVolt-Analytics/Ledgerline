"""Access-token refresh for the QuickBooks Online Accounting API."""
...
from app.integrations.qbo.store import get_qbo_integration
from app.models.accounting_integration import AccountingIntegration, AccountingIntegrationStatus
...
async def _refresh(db: AsyncSession, row: AccountingIntegration) -> None:
    ...
    if not prior_refresh:
        ...
        await db.flush()
        raise RuntimeError("QuickBooks refresh token missing")
    ...
    if response.status_code >= 400:
        ...
        if error_code == "invalid_grant":
            ...
            await db.flush()
            raise RuntimeError("QuickBooks connection requires re-authentication")
    ...
    if not access_token:
        raise RuntimeError("QuickBooks token refresh returned no access token")
...
async def get_valid_access_token(db: AsyncSession, tenant_id: uuid.UUID) -> str:
    row = await get_qbo_integration(db, tenant_id)
    ...
    token = decrypt_secret(row.access_token_encrypted)
    if not token:
        raise RuntimeError("QuickBooks access token unavailable after refresh")
    return token
