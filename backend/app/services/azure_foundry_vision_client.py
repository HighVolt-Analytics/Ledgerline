...
from app.services.llm_document_service import _normalize_llm_raw
from app.services.tenant_org_context import OrgContext
from app.services.vision_pdf import pdf_page_images
...
async def extract_fields_azure_foundry(
    ...
    *,
    ...
    if custom_keys:
        custom_hint = (
            f"\nAlso populate extracted_fields (or top-level keys) for tenant custom fields: "
            f"{', '.join(custom_keys)}."
        )
...
Use confirmed_dt as suggested_dt. Extract faithfully from the images.{custom_hint}"""
