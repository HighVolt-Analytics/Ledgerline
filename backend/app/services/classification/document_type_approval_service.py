...
from app.services.invoice.invoice_evaluation_service import EVAL_NEEDS_REVIEW, ROUTE_TEAM
from app.services.purchase.purchase_match_service import compute_three_way_match, load_purchase_order_for_invoice
    ...
    if mode == "touchless_on_clean_match":
        ...
        if match_mode_requires_po(match_mode):
            ...
            if so is not None:
                ...
                if match.status in {"3-Way Match", "2-Way Match"} or so.variance_approved:
                    return False
        invoice.status = InvoiceStatus.EXCEPTION
        invoice.evaluation_status = EVAL_NEEDS_REVIEW
        await log_event(
