...
from app.services.azure_foundry_vision_client import (
...
async def extract_fields(
    ...
    *,
    ...
    path = Path(file_path)
    if provider == DocumentAiProvider.GEMINI_VISION:
        return await extract_fields_gemini(
            path,
            org=org,
            document_types=document_types,
    ...
    if provider == DocumentAiProvider.AZURE_FOUNDRY_VISION:
        return await extract_fields_azure_foundry(
            path,
            org=org,
            document_types=document_types,
            confirmed_dt=confirmed_dt,
    ...
    enriched = await asyncio.to_thread(enrich_ocr_with_invoice_model, ocr, path)
