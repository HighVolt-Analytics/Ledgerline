...
from app.services.invoice_evaluation_service import ROUTE_SALES, parse_matched_rule_ids
from app.services.rule_book_mapper import load_classification_config, resolve_config_mapping
from app.services.sales_coding_service import code_so_from_invoice, inherit_so_coding_to_invoice
...
def compute_three_way_match(
    ...
    *,
    ...
    if fx is None and rule_book_config is not None:
        fx = resolve_fx_policy(config=rule_book_config, document_type_code=inv.document_type_code)
    if fx is not None and po_invoice_currency_mismatch(
        getattr(so, "so_currency", None),
        inv.currency,
        policy=fx,
    ):
...
async def list_sales_orders(
    ...
    from app.services.so_reference import is_plausible_so_reference
