...
def test_merge_institution_settings_non_pack_country_resets_stale_timezone():
    """Switching to a country without a jurisdiction pack must not keep US timezone."""
    merged = merge_institution_settings(
        {"country": "US", "timezone": "America/New_York", "locale": "en-US"},
        country="MM",
    )
    assert merged["country"] == "MM"
    assert merged["timezone"] == "Asia/Yangon"
    assert merged["locale"] in {"my-MM", "en-MM", "my"}
