...
export function buildBudgetUtilizationCsv(rows: EmployeeBudgetUtilizationRow[]): string {
  return rowsToCsv(
    ...
    rows.map((r) => [
      ...
      toNumber(r.advance_float),
      toNumber(r.monthly_cash_committed),
      blank(r.monthly_cash_remaining),
      blank(r.monthly_cash_utilization_pct),
      toNumber(r.quarterly_cash_committed),
      blank(r.quarterly_cash_remaining),
      blank(r.quarterly_cash_utilization_pct),
      toNumber(r.annual_cash_committed),
      blank(r.annual_cash_remaining),
      blank(r.annual_cash_utilization_pct),
      r.category_caps,
      r.claim_count,
      r.last_claim,
    ])
