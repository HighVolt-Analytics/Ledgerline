...
export function evaluationStatusDescription(
  ...
  if (status === "pending_vendor") {
    return isSales
      ? "Customer is not in master (VR12 on) — register in Contacts → Customers before processing."
      : "Vendor is not in master (VR12 on) — register in Contacts → Vendors before processing.";
  }
