...
> = {
  ...
  classify: { auditEvents: ["document_classified"] },
  bundle: { auditEvents: ["playbook_evaluated"] },
  vendor_hold: { auditEvents: ["vendor_registration_hold"] },
  validate: {
    auditEvents: ["validation_passed", "validation_failed", "routing_review_required"],
...
export function dossierStageDescription(stageId: DossierPipelineStageId): string {
  const descriptions: Record<DossierPipelineStageId, string> = {
    ...
    classify: "document_classified — DT code + confidence; low confidence may flag routing review.",
    ...
    validate: "validation_passed — VR01–VR16 incl. VR02 business duplicate; routing_review_required exits here.",
