...
import { needsReviewStatusChipClass } from "@/lib/kpiModuleColors";
    ...
    <Card className="p-4 mt-6" data-testid="live-eval">
      ...
      {!error && rows.length === 0 && !isLoading ? (
        <RuleBookEmptyPanel
          ...
          hint="Load documents from Upload to see live rule evaluation."
        />
      ) : (
                ...
                >
                  ...
                  <td className="px-3 py-2.5">
                    {row.email_rule ? (
                      ...
                      <Badge
                        variant="outline"
                        className="text-[11px] font-normal border-dashed ds-warning-text"
                      >
                  ...
                  <td className="px-3 py-2.5">
                    {row.vendor_match ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 text-xs whitespace-nowrap text-foreground">
                        <span
                          className="h-2 w-2 rounded-full bg-[#9c4e2a] dark:bg-[#edc0a6] shrink-0"
                          aria-hidden
                        />
                        Pending vendor
                      </span>
                  ...
                  <td className="px-3 py-2.5">
                    {row.category_rule ? (
                      ...
                      <div className="flex items-center gap-1.5">
                        <Badge
                          variant="outline"
                          className="text-[10px] font-normal border-dashed ds-warning-text"
                        >
                          {row.category_rule_disabled.kind}
                        </Badge>
                        <span className="text-xs ds-warning-text">
                  ...
                  <td className="px-3 py-2.5">
                    {row.auto_coded ? (
                      ...
                      <span className={needsReviewStatusChipClass()}>
                        <Clock className="approvals-action-chip__icon" />
                        Needs review
                      </span>
                    )}
                  </td>
