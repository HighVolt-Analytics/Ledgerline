...
export function BankFeedReconcileRow({
  ...
  const [activeTab, setActiveTab] = useState<ReconcileTab>(() => {
    if (txn.match_status === "suggested") return "match";
    const desc = (txn.description || "").toLowerCase();
    const looksLikeTransfer =
      /\btransfer\b/.test(desc) ||
      /\btrf\b/.test(desc) ||
      /\binternet transfer\b/.test(desc) ||
      /\bbetween accounts?\b/.test(desc);
    if (looksLikeTransfer && BANK_FEEDS_TRANSFER_ENABLED) return "transfer";
    return "create";
  });
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        description="It will leave matching. You can still see it on Bank Statement with status Excluded."
